...
my $sps = $ec2->DescribeSnapshots(DryRun => 0, Filters => [{Name => 'volume-id', Values => [$vols->Volumes->[0]->VolumeId]}]);
#p $sps;

foreach my $sp (@{$sps->Snapshots}){
#p $sp;
printf "--Snapshot: %30s %40s %25s %10s\n",$sp->SnapshotId,$sp->Description,$sp->StartTime,$sp->State;
}
}
